/**
 * ResetPasswordOtpScreen - Verify OTP for Password Reset
 * 
 * Spec:
 * - Title: "Verify it's you"
 * - Text: "Enter the 6-digit code we sent to your phone"
 * - Input: 6-digit OTP
 * - CTA: Verify, Resend (with timer)
 * - Error handling: wrong code, too many attempts
 */
...
import { verifyPasswordResetOtp, resendPasswordResetOtp, apiCall } from '../../services/authApi';
...
const ResetPasswordOtpScreen = () => {
  ...
  const { verificationId, maskedPhone, resendInSec: initialResendSec } = location.state || {};
  
  const [otp, setOtp] = useState(['', '', '', '', '', '']);
  const [error, setError] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [resendTimer, setResendTimer] = useState(initialResendSec || 30);
  ...
  const handleVerify = useCallback(async () => {
    ...
    try {
      const result = await apiCall(verifyPasswordResetOtp, currentVerificationId, code);
      
      trackEvent('otp_verified', { type: 'password_reset' });
      ...
      if (err.code === 'wrong_code') {
        ...
        setError(err.message || 'Verification code expired. Please request a new one.');
      } else if (err.code === 'too_many_attempts') {
        setError(err.message || 'Too many incorrect attempts. Please request a new code.');
  ...
  }, [otp, currentVerificationId, navigate]);
  ...
  const handleResend = async () => {
    ...
    try {
      const result = await apiCall(resendPasswordResetOtp, currentVerificationId);
      
      trackEvent('otp_sent', { type: 'password_reset_resend' });
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Enter the 6-digit code we sent to your phone{maskedPhone ? ` (${maskedPhone})` : ''}
          </Typography>
