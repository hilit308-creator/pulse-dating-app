...
import { verifyLoginOtp, resendLoginOtp, apiCall } from '../../services/authApi';
...
const LoginOtpVerificationScreen = () => {
  ...
  const handleVerify = async () => {
    ...
    try {
      const result = await apiCall(verifyLoginOtp, verificationId, code);
      ...
      if (err.code === 'wrong_code') {
        ...
        setError(err.message || 'Verification code expired. Please log in again.');
      } else if (err.code === 'too_many_attempts') {
        setError(err.message || 'Too many incorrect attempts. Please log in again.');
  ...
  const handleResend = async () => {
    ...
    try {
      const result = await apiCall(resendLoginOtp, verificationId);
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            We sent a verification code to your phone{maskedPhone ? ` (${maskedPhone})` : ''}
          </Typography>
